...
bool ApplicationClass::Initialize(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	result = m_TextureManager->LoadTexture(m_Direct3D->GetDevice(), L"Assets/dirt01d.dds", 0);
	if (!result)
	{
		return false;
	}

	result = m_TextureManager->LoadTexture(m_Direct3D->GetDevice(), L"Assets/dirt01n.dds", 1);
	if (!result)
	{
		return false;
	}
