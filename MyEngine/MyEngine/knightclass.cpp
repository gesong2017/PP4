...
KnightClass::KnightClass(const KnightClass& other)
{
}
...
bool KnightClass::Initialize(ID3D11Device* device, WCHAR* modelFilename, WCHAR* textureFilename)
{
	bool result;
...
void KnightClass::Render(ID3D11DeviceContext* deviceContext)
{
...
int KnightClass::GetInstanceCount()
{
	return m_instanceCount;
}

ID3D11ShaderResourceView* KnightClass::GetTexture()
{
	return m_Texture->GetTexture();
}

bool KnightClass::InitializeBuffers(ID3D11Device* device, WCHAR* modelFilename)
{
	...
	instances[0].position = XMFLOAT3(-2.5f, 0.0f, 2.5f);
	instances[1].position = XMFLOAT3(-2.5f, 0.0f, -2.5f);
	instances[2].position = XMFLOAT3(2.5f, 0.0f, 2.5f);
	instances[3].position = XMFLOAT3(2.5f, 0.0f, -2.5f);
	...
	hResult = device->CreateBuffer(&instanceBufferDesc, &instanceData, &m_instanceBuffer);
	if (FAILED(hResult))
	{
		return false;
	}
...
void KnightClass::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
...
bool KnightClass::LoadObjModel(WCHAR* fileName, vector<XMFLOAT3> &out_positions, vector<XMFLOAT2> &out_uvs, vector<XMFLOAT3> &out_normals)
{
...
bool KnightClass::LoadTexture(ID3D11Device* device, WCHAR* filename)
{
	bool result;
...
void KnightClass::ReleaseTexture()
{
