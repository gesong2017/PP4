...
bool SkyboxShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"SkyboxVertexShader.hlsl", L"SkyboxPixelShader.hlsl");
	if (!result)
	{
		return false;
	}
...
void SkyboxShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	fout.open("skyboxshader-error.txt");
	...
	MessageBox(hwnd, L"Error compiling shader.  Check skyboxshader-error.txt for message.", shaderFilename, MB_OK);
